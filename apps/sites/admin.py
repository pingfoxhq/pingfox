...
class SiteAdmin(admin.ModelAdmin):
    list_display = ('name', 'domain', 'site_id', 'is_verified', 'is_active', 'created_at')
    ...
    fieldsets = (
        (None, {
            'fields': ('user', 'name', 'domain', 'site_id')
        }),
        ('Status', {
            'fields': ('is_verified', 'is_active')
        }),
        ('Advanced Options', {
            'fields': ('pageview_limit_override',)
        }),
        ('Metadata', {
            'fields': ('created_at', 'timezone', 'verification_token')
        }),
    )
